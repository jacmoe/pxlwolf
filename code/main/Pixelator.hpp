...
public:
    ...
    void setSize(const std::string& name, Vector2i size);

    // Pixels
    void setPixel(const unsigned int x, const unsigned int y, const ALLEGRO_COLOR& color) { setPixel(m_current_buffer, x, y, color); }
    void setPixel(const std::string& name, unsigned int x, unsigned int y, const ALLEGRO_COLOR& color);
    void setPixelAlpha(const uint32_t x, const uint32_t y, const ALLEGRO_COLOR color, const double alpha) { setPixelAlpha(m_current_buffer, x, y, color, alpha); }
    void setPixelAlpha(const std::string& name, uint32_t x, uint32_t y, ALLEGRO_COLOR color, double alpha);
    ALLEGRO_COLOR getPixel(const unsigned int x, const unsigned int y) const { return getPixel(m_current_buffer, x, y); }
    ...
    void drawColumn(const unsigned int x, const unsigned int y, const unsigned int height, const ALLEGRO_COLOR& color) { drawColumn(m_current_buffer, x, y, height, color); }
    void drawColumn(const std::string& name, unsigned int x, unsigned int y, unsigned int height, const ALLEGRO_COLOR& color);
    void drawRow(const unsigned int x, const unsigned int y, const unsigned int length, const ALLEGRO_COLOR& color) { drawRow(m_current_buffer, x, y, length, color); }
    ...
    void drawRect(const std::string& name, IntRect rect, const ALLEGRO_COLOR& color);
    ...
    void drawCircle(const std::string& name, const Vector2i& coord, int radius, const ALLEGRO_COLOR& color);

    void fill(const ALLEGRO_COLOR color) { fill(m_current_buffer, color); }
    void fill(const std::string& name, ALLEGRO_COLOR color);
    void blendAlpha(const ALLEGRO_COLOR color, const double alpha) { blendAlpha(m_current_buffer, color, alpha); }
    ...
    bool addBuffer(const std::string& name, int width, int height);
    bool removeBuffer(const std::string& name);
    void setActiveBuffer(const std::string& name);
    std::string getActiveBuffer() { return m_current_buffer; }
    ...
    void copy(std::string name, unsigned int x = 0, unsigned int y = 0, bool applyAlpha = true);
    void copy(std::string name, unsigned int destX, unsigned int destY, const IntRect& sourceRect, bool applyAlpha = true);
    //void copy(const sf::Image& source, unsigned int destX, unsigned int destY, const IntRect& sourceRect, bool applyAlpha = false);
    void copy(const uint8_t* source_pixels, Vector2i buffer_size, unsigned int destX, unsigned int destY, const IntRect& sourceRect, bool applyAlpha = true);

    // get the size (width and height) of a buffer
    IntRect getSize() { return getSize(m_current_buffer); }
    IntRect getSize(std::string name);
