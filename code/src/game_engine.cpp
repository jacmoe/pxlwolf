...
void GameEngine::initPlayer(Player* player, double x, double y, double angle, uint8_t usingMouse, double fov, double viewDist, uint32_t screenWidth)
{
	player->x = x;
	player->y = y;
	player->h = 0;
	...
	RaycasterEngine::generateAngleValues(screenWidth, &(player->camera));
}

void GameEngine::initEntity(Entity* entity, double x, double y, double h, double angle, RayTex* spriteTex, RayTex* shadowTex)
{
	entity->x = x;
	entity->y = y;
	entity->h = h;
	...
	RaycasterEngine::initSprite(&(entity->sprite), spriteTex, 1.0, 1.0, x, y, h);
	RaycasterEngine::initSprite(&(entity->shadow), shadowTex, 0, 0.5, x, y, -0.5);
}
...
void GameEngine::moveEntity(Entity* entity, double x, double y, double h)
{
	entity->x = x;
	entity->y = y;
	entity->h = h;
...
void GameEngine::scaleEntity(Entity* entity, double scaleFactor)
{
	entity->sprite.scaleFactor = scaleFactor;
}

void GameEngine::updatePlayer(Player* player, Map* map, KeyMap* keyMap, double dt)
{
	int borderWidth = 2;
...
void GameEngine::updateEntity(Entity* entity)
{
...
void GameEngine::updateProjectile(ProjectileList* projectiles, uint32_t numProjectile, Player* player)
{
	double velocity = 0.1;
	...
	if (keys[SDL_SCANCODE_SPACE] && !player->spacePressed)
	{
		for (int i = 0; i < numProjectile; i++)
		{
			...
			{
				GameEngine::moveEntity(&projectiles->projectiles[i], player->x - 0.1 * sin(player->angle), player->y + 0.1 * cos(player->angle), -0.1);
	...
	for (int i = 0; i < numProjectile; i++)
	{
		if (projectiles->projectiles[i].state == 1)
		{
			...
			GameEngine::updateEntity(&projectiles->projectiles[i]);
		}
	}
}

void GameEngine::bindKeys(KeyMap* keyMap, uint8_t* keyList)
{
	int i = 0;
...
void GameEngine::updateKeys(KeyMap* keyMap)
{
	const uint8_t* keys = SDL_GetKeyboardState(NULL);
